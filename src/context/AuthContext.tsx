...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        // Initialize device tracking and anti-fraud for logged in users
  ...
  const signup = async (email: string, password: string, username: string): Promise<UserCredential> => {
    ...
    const existingCheck = await getDoc(doc(usersRef, 'device_index', deviceInfo.fingerprint));
    ...
    await setDoc(doc(db, 'users', result.user.uid, 'devices', 'current'), {
      deviceId: deviceInfo.deviceId,
      fingerprint: deviceInfo.fingerprint,
